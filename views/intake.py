...
set_page_wonk("high")  # one calm, focused act - nothing else competing for attention
...
if extraction:
    ...
    remote_anywhere = st.checkbox(
        "\U0001f30d Remote from anywhere — no UK work authorization required",
        key="remote_anywhere_flag",
        help=(
            "Only check this if the employer explicitly allows hiring from any location, "
            "with no requirement to hold UK right-to-work. A UK-based remote role still "
            "needs standard sponsorship - leave this unchecked for those."
        ),
    )

    resolved_employer = None
    verdict = None

    if remote_anywhere:
        resolved_employer = extraction.company_name or "(remote - any location)"
        verdict = SponsorVerdict(status=SKIPPED, reason=REMOTE_ANYWHERE_SKIP_REASON, matched_name=resolved_employer)
    elif not employer_query:
        st.warning(
    ...
    if resolved_employer and verdict:
